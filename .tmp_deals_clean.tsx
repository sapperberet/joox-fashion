  ...
  return (
    <main className="mx-auto flex max-w-6xl flex-col gap-12 px-6 py-16">
      <div className="flex flex-wrap items-start justify-between gap-4 rounded-3xl border border-gold/20 bg-stone/80 p-6">
        <Link href={`/atelier?admin_token=${encodeURIComponent(adminToken)}`}>
          <button className="text-sm text-gold/70 hover:text-gold">
            ÔåÉ {isArabic ? "Ïº┘äÏ╣┘êÏ»Ï®" : "Back"}
          </button>
        </Link>
        <div>
          <h1 className="font-display text-3xl tracking-[0.2em] text-gold">
            {isArabic ? "ÏÑÏ»ÏºÏ▒Ï® Ïº┘äÏ╣Ï▒┘êÏÂ" : "Deal Management"}
          </h1>
          <p className="mt-2 text-sm text-sand/70">
            {isArabic ? "Ïú┘åÏ┤Ïª ┘êÏúÏ»Ï▒ Ï╣Ï▒┘êÏÂ Ï┤Ï▒ÏºÏí X ┘êÏº┘äÏ¡ÏÁ┘ê┘ä Ï╣┘ä┘ë Y ┘àÏ¼Ïº┘å┘è" : "Create and manage buy X get Y free deals"}
          </p>
        </div>
      ...
      {!isCreating && !editingId && (
        ...
        >
          {isArabic ? "ÏÑ┘åÏ┤ÏºÏí Ï╣Ï▒ÏÂ Ï¼Ï»┘èÏ»" : "Create New Deal"}
        </button>
      )}
      ...
      {(isCreating || editingId) && (
        ...
        >
          ...
          <h2 className="font-display text-xl tracking-[0.2em] text-gold">
            {editingId ? (isArabic ? "Ï¬Ï¡Ï▒┘èÏ▒ Ïº┘äÏ╣Ï▒ÏÂ" : "Edit Deal") : (isArabic ? "ÏÑ┘åÏ┤ÏºÏí Ï╣Ï▒ÏÂ Ï¼Ï»┘èÏ»" : "Create New Deal")}
          </h2>

          <div className="rounded-2xl border border-gold/15 bg-obsidian/40 p-4">
            <p className="text-xs text-sand/70 font-semibold mb-3">
              {isArabic ? "┘àÏ╣┘ä┘ê┘àÏºÏ¬ ÏúÏ│ÏºÏ│┘èÏ®" : "Basic Information"}
            </p>
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                <label className="block text-xs font-semibold text-gold/70 mb-1">
                  {isArabic ? "Ïº┘äÏºÏ│┘à (ÏÑ┘åÏ¼┘ä┘èÏ▓┘è)" : "Name (English)"}
                </label>
                <input
                  type="text"
                  name="name_en"
              ...
              <div>
                <label className="block text-xs font-semibold text-gold/70 mb-1">
                  {isArabic ? "Ïº┘äÏºÏ│┘à (Ï╣Ï▒Ï¿┘è)" : "Name (Arabic)"}
                </label>
                <input
                  type="text"
                  name="name_ar"
                  ...
                  placeholder="┘àÏ½Ïº┘ä: Ï╣Ï▒ÏÂ Ïº┘äÏÁ┘è┘ü"
                  className="w-full rounded-2xl border border-gold/20 bg-obsidian px-4 py-3 text-sm text-sand"
                  required
                />
              </div>
          ...
          <div className="rounded-2xl border border-gold/15 bg-obsidian/40 p-4">
            <p className="text-xs text-sand/70 font-semibold mb-3">
              {isArabic ? "┘å┘êÏ╣ Ïº┘äÏ╣Ï▒ÏÂ" : "Deal Type"}
            </p>
            <select
              name="deal_type"
            ...
            >
              <option value="buy_x_get_y">
                {isArabic ? "ÏºÏ┤Ï¬Ï▒┘è X ┘êÏºÏ¡ÏÁ┘ä Ï╣┘ä┘ë Y ┘àÏ¼Ïº┘å┘è (Ïú┘è ┘à┘åÏ¬Ï¼ÏºÏ¬)" : "Buy X Get Y Free (Any Products)"}
              </option>
              <option value="buy_x_of_product_get_y_free">
                {isArabic ? "ÏºÏ┤Ï¬Ï▒┘è X ┘à┘å ┘à┘åÏ¬Ï¼ ┘àÏ╣┘è┘å ┘êÏºÏ¡ÏÁ┘ä Ï╣┘ä┘ë Y ┘àÏ¼Ïº┘å┘è" : "Buy X of Specific Product Get Y Free"}
              </option>
            </select>
            <p className="text-xs text-sand/50 mt-2">
              {isTriggerType
                ? isArabic
                  ? "Ï¡Ï»Ï» Ïº┘ä┘à┘åÏ¬Ï¼ÏºÏ¬ Ïº┘äÏ¬┘è ┘èÏ¼Ï¿ Ï┤Ï▒ÏºÏñ┘çÏº ┘äÏ¬┘üÏ╣┘è┘ä Ïº┘äÏ╣Ï▒ÏÂ"
                  : "Select products that must be purchased to trigger the deal"
                : isArabic
                  ? "Ï│┘èÏ¬┘à Ï¬ÏÀÏ¿┘è┘é Ïº┘äÏ╣Ï▒ÏÂ Ï¬┘ä┘éÏºÏª┘èÏº┘ï Ï╣┘åÏ» Ïº┘äÏ┤Ï▒ÏºÏí ┘à┘å Ïº┘ä┘à┘åÏ¬Ï¼ÏºÏ¬ Ïº┘ä┘àÏ«Ï¬ÏºÏ▒Ï®"
          ...
          <div className="rounded-2xl border border-gold/15 bg-obsidian/40 p-4">
            <p className="text-xs text-sand/70 font-semibold mb-3">
              {isArabic ? "Ï┤Ï▒┘êÏÀ Ïº┘äÏ╣Ï▒ÏÂ" : "Deal Conditions"}
            </p>
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                <label className="block text-xs font-semibold text-gold/70 mb-1">
                  {isArabic ? "Ï╣Ï»Ï» Ïº┘äÏ┤Ï▒ÏºÏí" : "Buy Quantity"}
                </label>
                <input
                  type="number"
                  name="buy_quantity"
                ...
                <p className="text-xs text-sand/50 mt-1">
                  {isArabic ? "┘â┘à ┘èÏ¼Ï¿ Ïú┘å ┘èÏ┤Ï¬Ï▒┘è Ïº┘äÏ╣┘à┘è┘ä" : "How many items customer must buy"}
                </p>
              </div>
              <div>
                <label className="block text-xs font-semibold text-gold/70 mb-1">
                  {isArabic ? "Ï╣Ï»Ï» Ïº┘ä┘àÏ¼Ïº┘å┘è" : "Free Quantity"}
                </label>
                <input
                  type="number"
                  name="free_quantity"
                ...
                />
                <p className="text-xs text-sand/50 mt-1">
                  {isArabic ? "┘â┘à ┘èÏ¡ÏÁ┘ä Ï╣┘ä┘ë ┘àÏ¼Ïº┘å┘è" : "How many free items customer gets"}
                </p>
              </div>
            </div>
          </div>
          ...
          {isTriggerType && (
            <div className="rounded-2xl border border-gold/15 bg-obsidian/40 p-4">
              <p className="text-xs text-sand/70 font-semibold mb-2">
                {isArabic ? "┘à┘åÏ¬Ï¼ÏºÏ¬ Ïº┘äÏ¬┘üÏ╣┘è┘ä (Ïº┘ä┘àÏÀ┘ä┘êÏ¿Ï®)" : "Trigger Products (Must Buy)"}
              </p>
              <p className="text-xs text-sand/50 mb-3">
                {isArabic
                  ? "Ï¡Ï»Ï» Ïº┘ä┘à┘åÏ¬Ï¼ÏºÏ¬ Ïº┘äÏ¬┘è ┘èÏ¼Ï¿ Ï┤Ï▒ÏºÏí Ïº┘äÏ╣┘à┘è┘ä ┘â┘à┘èÏºÏ¬ ┘à┘å┘çÏº ┘äÏ¬┘üÏ╣┘è┘ä ┘çÏ░Ïº Ïº┘äÏ╣Ï▒ÏÂ"
              ...
              <div className="max-h-64 overflow-y-auto space-y-1 border border-gold/20 rounded-lg p-3 bg-obsidian/30">
                {products.length === 0 ? (
                  <p className="text-xs text-sand/50 py-4 text-center">
                    {isArabic ? "┘äÏº Ï¬┘êÏ¼Ï» ┘à┘åÏ¬Ï¼ÏºÏ¬" : "No products"}
                  </p>
                ) : (
              ...
              {!hasValidTrigger && (
                <p className="text-xs text-red-400 mt-2">
                  {isArabic ? "┘èÏ▒Ï¼┘ë Ï¬Ï¡Ï»┘èÏ» ┘à┘åÏ¬Ï¼ ┘êÏºÏ¡Ï» Ï╣┘ä┘ë Ïº┘äÏú┘é┘ä" : "Please select at least one product"}
                </p>
              )}
            </div>
          )}
          ...
          <div className="rounded-2xl border border-gold/15 bg-obsidian/40 p-4">
            <p className="text-xs text-sand/70 font-semibold mb-2">
              {isArabic ? "Ïº┘ä┘à┘åÏ¬Ï¼ÏºÏ¬ Ïº┘ä┘àÏÀÏ¿┘éÏ® (Ïº┘ä┘àÏ¼Ïº┘å┘èÏ®)" : "Applicable Products (Get Free)"}
            </p>
            <p className="text-xs text-sand/50 mb-3">
              {isArabic
                ? "Ï¡Ï»Ï» Ïº┘ä┘à┘åÏ¬Ï¼ÏºÏ¬ Ïº┘äÏ¬┘è ┘è┘à┘â┘å Ïú┘å ┘èÏ¡ÏÁ┘ä Ï╣┘ä┘è┘çÏº Ïº┘äÏ╣┘à┘è┘ä ┘àÏ¼Ïº┘åÏº┘ï"
          ...
          <div className="flex gap-2">
            ...
            >
              {editingId ? (isArabic ? "Ï¬Ï¡Ï»┘èÏ½" : "Update") : (isArabic ? "ÏÑ┘åÏ┤ÏºÏí" : "Create")}
            </button>
            <button
              type="button"
              onClick={handleReset}
      ...
      <div className="grid gap-4">
        <h3 className="font-display text-xl tracking-[0.2em] text-gold">
          {isArabic ? "Ïº┘äÏ╣Ï▒┘êÏÂ Ïº┘ä┘åÏ┤ÏÀÏ®" : "Active Deals"} ({deals.filter((d) => d.is_active).length}/{deals.length})
        </h3>
        {deals.length === 0 ? (
          <div className="rounded-2xl border border-gold/15 bg-obsidian/40 p-8 text-center">
            <p className="text-sand/50">
              {isArabic ? "┘äÏº Ï¬┘êÏ¼Ï» Ï╣Ï▒┘êÏÂ Ï¿Ï╣Ï»" : "No deals yet"}
            </p>
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-sand/50 mt-2 space-y-1">
                      <p>
                        {isArabic ? "Ïº┘ä┘à┘åÏ¬Ï¼ÏºÏ¬ Ïº┘ä┘àÏÀÏ¿┘éÏ®:" : "Applicable:"} {(deal.applicable_product_ids as string[])?.length || 0}{" "}
                        {isArabic ? "┘à┘åÏ¬Ï¼" : "products"}
                      </p>
                      {deal.deal_type === "buy_x_of_product_get_y_free" && (
                        <p>
                          {isArabic ? "┘à┘åÏ¬Ï¼ÏºÏ¬ Ïº┘äÏ¬┘üÏ╣┘è┘ä:" : "Triggers:"} {(deal.trigger_product_ids as string[])?.length || 0}{" "}
                          {isArabic ? "┘à┘åÏ¬Ï¼" : "products"}
                        </p>
                      )}
                    </div>
                  ...
                  <div className="flex flex-col gap-2">
                    <form action={toggleDealStatus} className="inline">
                      ...
                      >
                        {deal.is_active ? (isArabic ? "┘åÏ┤ÏÀ" : "Active") : (isArabic ? "┘àÏ╣ÏÀ┘ä" : "Inactive")}
                      </button>
                    </form>
                    <button
                      type="button"
