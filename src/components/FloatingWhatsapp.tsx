...
export default function FloatingWhatsapp() {
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className={`fixed bottom-6 z-50 inline-flex items-center gap-2 rounded-full bg-emerald px-4 py-3 text-sm font-semibold text-ink shadow-2xl transition hover:scale-[1.05] ${
        locale === "ar" ? "left-6" : "right-6"
      }`}
      aria-label={t.sections.wholesale}
    >
      <span>𓂀</span>
      <span className="text-xs uppercase tracking-[0.2em]">{t.wholesale.cta}</span>
    </a>
  );
}
