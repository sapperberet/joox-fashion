...
export default function ProductClient({
  ...
  const [liked, setLiked] = useState(() => (product ? hasWishlistItem(product.slug) : false));
  ...
  useEffect(() => {
    ...
    let mounted = true;
    fetchProductReviews(product.slug).then((storedReviews) => {
      if (!mounted) {
        return;
      }
      ...
      <main className="mx-auto flex max-w-6xl flex-col gap-10 sm:gap-16 px-4 sm:px-6 py-8 sm:py-20">
        {(category || subcategory) && (
          <div className="mb-2">
            <Breadcrumb
              items={[
                ...(category ? [{ label: locale === "ar" ? category.name_ar : category.name_en, href: `/products?category=${category.slug}` }] : []),
                ...(subcategory ? [{ label: locale === "ar" ? subcategory.name_ar : subcategory.name_en, href: `/products?subcategory=${subcategory.slug}` }] : []),
        ...
        <section className="grid gap-6 sm:gap-10 lg:grid-cols-2">
          ...
          <div className="flex flex-col gap-4 sm:gap-6">
            ...
            {variants.length > 0 && (
              <div className="grid gap-4 rounded-2xl border border-gold/20 bg-obsidian/50 p-4 sm:p-5">
                <div className="text-xs uppercase tracking-[0.25em] text-gold/70 font-semibold">Colors</div>
                ...
                <div className="text-xs uppercase tracking-[0.25em] text-gold/70 font-semibold">Sizes</div>
                <div className="flex flex-wrap gap-2">
            ...
            <div className="flex flex-col sm:flex-row flex-wrap gap-2 sm:gap-3 pt-2 sm:pt-4">
              ...
              >
                𓋹 {cartAction === "checkout" ? t.products.addedToCheckout : t.products.order}
              </button>
              <button
                type="button"
              ...
              >
                𓋹 {cartAction === "basket" ? t.products.addedToBasket : t.products.addToBasket}
              </button>
              <Link
                href="/products"
          ...
          <section className="flex flex-col gap-6 sm:gap-8 border-t-2 border-gold/20 pt-8 sm:pt-12">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
              <h2 className="font-display text-xl sm:text-2xl tracking-[0.2em] text-gold flex items-center gap-2">
                <span>𓋹</span> Related items <span>𓂀</span>
              </h2>
              <Link
                href="/products"
          ...
          <section className="flex flex-col gap-6 border-t-2 border-gold/20 pt-8 sm:pt-12">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
              <h2 className="font-display text-xl sm:text-2xl tracking-[0.2em] text-gold flex items-center gap-2">
                <span>𓋹</span> People also buy <span>𓂀</span>
              </h2>
              <p className="text-xs uppercase tracking-[0.3em] text-sand/60">Complete the look</p>
            </div>
            <div className="grid gap-4 sm:gap-6 md:grid-cols-2 lg:grid-cols-4">
          ...
          <section className="flex flex-col gap-4 border-t-2 border-gold/20 pt-8 sm:pt-12">
            <div>
              <h2 className="font-display text-xl sm:text-2xl tracking-[0.2em] text-gold flex items-center gap-2">
                <span>𓋹</span> FAQ <span>𓂀</span>
              </h2>
              <p className="mt-2 text-sm text-sand/60">Product fit, delivery, returns, and support</p>
            </div>
          ...
          <section className="flex flex-col gap-6 border-t-2 border-gold/20 pt-8 sm:pt-12">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <h2 className="font-display text-xl sm:text-2xl tracking-[0.2em] text-gold flex items-center gap-2">
                  <span>★</span> Ratings & Reviews <span>☆</span>
                </h2>
                <p className="mt-2 text-sm text-sand/60">
                  {reviewSummary.average > 0 ? `${reviewSummary.average.toFixed(1)} out of 5` : "No reviews yet"} · {reviewSummary.count} reviews
                </p>
              </div>
              ...
              <form onSubmit={handleSubmitReview} className="grid gap-3 rounded-3xl border border-gold/20 bg-obsidian/60 p-4 sm:p-5">
                <div className="grid gap-3 sm:grid-cols-2">
                  <input
                    ...
                    className="rounded-xl border border-gold/20 bg-obsidian px-4 py-3 text-sand placeholder:text-sand/40"
                  />
                  <input
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {Array.from({ length: 5 }, (_, index) => index + 1).map((star) => (
                    <button
                      ...
                      className={`text-2xl transition ${star <= reviewRating ? "text-gold" : "text-sand/30"}`}
                ...
                <div className="flex flex-wrap items-center gap-3">
                  {submittingReview ? (
                    <div className="text-xs uppercase tracking-[0.25em] text-gold/70">{locale === "ar" ? "جاري الإرسال..." : "Submitting..."}</div>
                  ) : null}
                  <button
                    type="submit"
                  ...
                  >
                    {submittingReview ? (locale === "ar" ? "جاري الإرسال..." : "Submitting...") : (locale === "ar" ? "إرسال التقييم" : "Post review")}
                  </button>
                  <Link href="/auth" className="rounded-full border border-gold/20 px-5 py-3 text-xs font-semibold uppercase tracking-[0.2em] text-gold">
                    Sign in
                  </Link>
